...
$tableID.on("click", ".table-down", function() {
	...
	$(".file-upload").on("change", function() {
		...
		! function(e) {
			if (e.files) {
				let dom="";
				$(parent).find("#image-render-area").html("");
				for(let i=0;i<e.files.length;i++){
					let t = new FileReader;
					t.onload = function(e) {
						dom=`<div class="childAvt"><img class="img-fluid imaUserCustomClone"  src="`+e.target.result+`"
                           alt="profile-pic">
						`;
						if(i===0){
							dom+=`<p style="color: black;">Ảnh đại diện</p></div>`
						}else{
							dom+=`<p style="color: black;">Ảnh thứ `+i+`</p></div>`
						}
						$(parent).find("#image-render-area").append(dom);
	...
	$(".img-upload").on("change", function() {
		let parent =$(this).closest(".add-img-user");
		! function(e) {
